...
This repo contains the source code of the Python package `lora` and several examples of how to integrate it with practical models such as those in HuggingFace.
...
LoRA also outperforms several other adaptation methods including adapters, prefix-tuning, and fine-tuning.
...
On GPT-2, LoRA compares favorably to both full finetuning and other efficient tuning methods, such as [adapter (Houlsby et al., 2019)](https://arxiv.org/abs/1902.00751) and [prefix tuning (Li and Liang, 2021)](https://arxiv.org/abs/2101.00190). Using E2E NLG Challenge as an example:
|   | Method              | # of Trainable Params | E2E (BLEU)   | DART (BLEU)  | WebNLG (BLEU-U/S/A)   |
|---|---------------------|-----------------------|--------------|--------------|-----------------------|
|   | GPT-2 M (Fine-Tune) | 354.92M               | 68.2         | 46.0         | 30.4/<b>63.2</b>/47.6        |
|   | GPT-2 M (Adapter)   | 11.48M                | 68.9         | 45.4         | <b>47.9</b>/61.1/<b>55.2</b> |
|   | GPT-2 M (Prefix)    | 0.35M                 | 69.7         | 45.7         | 44.1/63.1/54.4        |
|   | GPT-2 M (LoRA)      | 0.35M                 |<b>70.4</b>±.1|<b>47.1</b>±.2| 46.7±.4/62.1±.2/<b>55.3</b>±.2 |
|   | GPT-2 L (Fine-Tune) | 774.03M               | 68.5         | 46.5         | 41.7/<b>64.6</b>/54.2        |
|   | GPT-2 L (Prefix)    | 0.77M                 | 70.3         | 46.5         | 47.0/64.2/56.4        |
|   | GPT-2 L (LoRA)      | 0.77M                 |<b>70.4</b>±.1|<b>47.5</b>±.1| <b>48.4</b>±.3/64.0±.3/<b>57.0</b>±.1 |

Non-LoRA baselines are taken from [Li and Liang (2021)](https://arxiv.org/abs/2101.00190).
...
```
3. Training bias vectors in tandem with LoRA might be a cost-efficient way to squeeze out extra task performance (if you tune the learning rate carefully). While we did not study its effect thoroughly in our paper, we make it easy to enable it in `lora`. You can mark some biases as trainable by passing "all" or "lora_only" to `bias=` when calling `mark_only_lora_as_trainable`. Remember to pass the corresponding `bias=` argument to `lora_state_dict` when saving a checkpoint.
```
# ===== Before =====
...
For questions related to the library `lora`:
