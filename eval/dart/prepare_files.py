...
def prepare_files_ter(system_output, reference_file0, reference_file1, reference_file2):
    ...
    with open(temp_home+'all-notdelex-refs-ter.txt', 'w+') as f:
        for ref in references:
            f.write(''.join(ref))
    ...
    print(temp_home+'relexicalised_predictions-ter.txt')
    with open(temp_home+'relexicalised_predictions-ter.txt', 'w+') as f:
        f.write(''.join(geners))
    ...
    with open(temp_home+'all-notdelex-refs-meteor.txt', 'w+') as f:
...
if __name__ == "__main__":
    temp_home = os.path.expanduser("~/temp_eval/")
